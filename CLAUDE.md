...
- `input :name do required(:name).value(...) end` - Declare required input with dry-schema validation (the `Dry::Schema.Params` wrapper is implicit)
- `input :name, value: :type, predicate?: ...` - Block-less short form for simple validation; accepts a key path (`input [:a, :b], value: :integer`) which expands into nested `required(:a).hash { required(:b).value(...) }`
- `dependency :fact_name, from: :module_name` - Declare dependency on another fact
- A resolver proc that computes the fact value using `data` (a DataContainer). If a fact has exactly one `input`, no `dependency`, and no `proc`, the resolver is synthesized as `proc { data.dig(:input, *keypath) }`. If a fact has inputs/dependencies but no callable resolver, `Fact#initialize` raises.
