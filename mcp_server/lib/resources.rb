...
module Resources
  ...
  TEMPLATES = [
    ...
    MCP::ResourceTemplate.new(
      ...
      name: "example",
      description: "A specific example (simple_income, snap_benefits, household_members, tax_credit, multi_graph, progressive_screening)",
      mime_type: "text/markdown"
    )
  ]
  ...
  EXAMPLES = {
    ...
    "multi_graph" => {
      title: "Multi-Graph with Shared and Context-Specific Facts",
      description: "Multiple graph contexts that share common facts but have different constants and rules",
      patterns: ["graph contexts", "shared facts", "context-specific facts", "ActiveSupport::Concern", "in_module"],
      policy: <<~POLICY,
        State child tax credits with shared eligibility logic but state-specific rules:

        Common rules (all states):
        - Child must be under max_age (varies by state)
        - Must have qualifying relationship (child, stepchild, foster child)
        - Child must live with taxpayer for 6+ months of the year

        Colorado (2025):
        - max_age: 17 (children under 17 qualify)
        - Credit amount: $1,200 per qualifying child

        New Jersey (2024):
        - max_age: 6 (children under 6 qualify)
        - Credit amount: $500 per qualifying child

        This demonstrates:
        - `add_graph_context` to create separate evaluation contexts
        - Shared facts (no graph_context) become ActiveSupport::Concern modules
        - Context-specific facts with different constants per state/year
        - Each context evaluates independently with its own rules
      POLICY
      code: <<~RUBY
        # Generated by MCP when using graph contexts:
        #
        # 1. Call add_graph_context("co_2025") and add_graph_context("nj_2024")
        # 2. Add shared facts without graph_context parameter
        # 3. Add context-specific facts with graph_context: "co_2025" or "nj_2024"
        #
        # The export_code tool generates this structure:

        # Shared eligibility logic as a Concern
        module DependentsFacts
          extend ActiveSupport::Concern

          included do
            in_module :dependents do
              fact :age, per_entity: :dependents do
                input :date_of_birth, per_entity: true do
                  Dry::Schema.Params { required(:date_of_birth).value(:date) }
                end
                dependency :tax_year, from: :filing_context

                proc do
                  data in input: { date_of_birth: }, dependencies: { tax_year: }
                  tax_year - date_of_birth.year
                end
              end

              fact :has_qualifying_relationship, per_entity: :dependents do
                input :relationship, per_entity: true do
                  Dry::Schema.Params do
                    required(:relationship).value(:string,
                      included_in?: %w[child stepchild foster_child grandchild sibling])
                  end
                end

                proc do
                  data in input: { relationship: }
                  %w[child stepchild foster_child].include?(relationship)
                end
              end

              fact :meets_residency, per_entity: :dependents do
                input :months_lived_with_taxpayer, per_entity: true do
                  Dry::Schema.Params { required(:months_lived_with_taxpayer).value(:integer, gteq?: 0, lteq?: 12) }
                end

                proc do
                  data in input: { months_lived_with_taxpayer: }
                  months_lived_with_taxpayer >= 6
                end
              end

              # Shared qualification logic - uses max_age from filing_context (context-specific)
              fact :qualifies_for_state_credit, per_entity: :dependents do
                dependency :age
                dependency :has_qualifying_relationship
                dependency :meets_residency
                dependency :max_age, from: :filing_context

                proc do
                  data in dependencies: { age:, has_qualifying_relationship:, meets_residency:, max_age: }
                  age < max_age && has_qualifying_relationship && meets_residency
                end
              end
            end
          end
        ...
        # Colorado 2025 context
        class Co2025Graph < FactGraph::Graph; end

        class Co2025Facts < Co2025Graph
          include DependentsFacts

          in_module :filing_context do
            constant(:tax_year) { 2025 }
            constant(:max_age) { 17 }
            constant(:credit_per_child) { 1200 }
          end

          in_module :benefits do
            fact :qualifying_child_count do
              dependency :qualifies_for_state_credit, from: :dependents

              proc do
                results = data[:dependencies][:qualifies_for_state_credit]
                results.values.count { |v| v == true }
              end
            end

            fact :total_credit do
              dependency :qualifying_child_count
              dependency :credit_per_child, from: :filing_context

              proc do
                data in dependencies: { qualifying_child_count:, credit_per_child: }
                qualifying_child_count * credit_per_child
              end
            end
          end
        end

        # New Jersey 2024 context
        class Nj2024Graph < FactGraph::Graph; end

        class Nj2024Facts < Nj2024Graph
          include DependentsFacts

          in_module :filing_context do
            constant(:tax_year) { 2024 }
            constant(:max_age) { 6 }
            constant(:credit_per_child) { 500 }
          end
        ...
        # input = {
        #   dependents: [
        #     { date_of_birth: Date.new(2020, 5, 15), relationship: "child", months_lived_with_taxpayer: 12 },
        #     { date_of_birth: Date.new(2015, 3, 10), relationship: "child", months_lived_with_taxpayer: 12 }
        #   ]
        # }
        #
        # co_results = FactGraph::Evaluator.evaluate(input, graph_class: Co2025Graph)
        # # Both children qualify (ages 5 and 10, both under 17)
        # # => total_credit: 2400
        #
        # nj_results = FactGraph::Evaluator.evaluate(input, graph_class: Nj2024Graph)
        # # Only first child qualifies (age 4, under 6; second is 9, not under 6)
        # # => total_credit: 500
      RUBY
    },
  ...
  DSL_DOCS = <<~MARKDOWN
    ...
    **Result structure:**
    ...
    ## Multi-Graph Contexts

    When you need multiple versions of a fact graph (e.g., different states or tax years), use graph contexts to share common facts while having context-specific constants and rules.

    ### Setting Up Graph Contexts

    Use `add_graph_context` to create separate evaluation contexts:

    ```ruby
    # In MCP:
    # add_graph_context(name: "co_2025")
    # add_graph_context(name: "nj_2024")
    ```

    ### Shared vs Context-Specific Facts

    - **Shared facts**: Add without `graph_context` parameter - these become `ActiveSupport::Concern` modules
    - **Context-specific facts**: Add with `graph_context: "co_2025"` - these go in context-specific classes

    ```ruby
    # Shared fact (no graph_context) - reused by all contexts
    add_fact(name: "qualifies", module_name: "eligibility", ...)

    # Context-specific constant (different values per context)
    add_fact(name: "max_age", module_name: "limits", constant_value: "17", graph_context: "co_2025")
    add_fact(name: "max_age", module_name: "limits", constant_value: "6", graph_context: "nj_2024")
    ```

    ### Generated Code Structure

    The `export_code` tool generates:

    ```ruby
    # Shared facts as Concern
    module EligibilityFacts
      extend ActiveSupport::Concern
      included do
        in_module :eligibility do
          fact :qualifies do ... end
        end
      end
    end

    # Context-specific base class (separate registry)
    class Co2025Graph < FactGraph::Graph; end

    # Context-specific facts class
    class Co2025Facts < Co2025Graph
      include EligibilityFacts  # Include shared facts

      in_module :limits do
        constant(:max_age) { 17 }  # CO-specific
      end
    end
    ...
    class Nj2024Facts < Nj2024Graph
      include EligibilityFacts

      in_module :limits do
        constant(:max_age) { 6 }  # NJ-specific
      end
    end
    ```
    ...
    Each context has its own graph registry and is evaluated independently:

    ```ruby
    # Same input, different results based on context rules
    co_results = FactGraph::Evaluator.evaluate(input, graph_class: Co2025Graph)
    nj_results = FactGraph::Evaluator.evaluate(input, graph_class: Nj2024Graph)
    ```

    See `factgraph://examples/multi_graph` for a complete example.
    ...
    **Use cases:**
    ...
    - `factgraph://examples/multi_graph` - Multiple contexts with shared and context-specific facts
