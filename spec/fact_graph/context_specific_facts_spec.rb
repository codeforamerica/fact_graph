RSpec.describe "Context-specific facts" do
  ...
  it "allows separate graphs to contain some shared and some specific facts with correct module names" do
    expect(Ty2024Graph.prepare_fact_objects({})[:filer][:age].call({}, {})).to eq Dry::Monads::Success(34)
    expect(Ty2024Graph.prepare_fact_objects({})[:dependent][:age].call({}, {})).to eq Dry::Monads::Success(24)

    expect(Ty2025Graph.prepare_fact_objects({})[:filer][:age].call({}, {})).to eq Dry::Monads::Success(35)
    expect(Ty2025Graph.prepare_fact_objects({})[:dependent][:age].call({}, {})).to eq Dry::Monads::Success(25)
